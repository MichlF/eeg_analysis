...
def runWholeSession(ra, Ea,session):
	...
	# select bad epochs
	session = RejectEpochs(session,session.event_list,Ea[0]['event_id_mem'],Ea[1]['timing_mem'][0],Ea[1]['timing_mem'][1],None)
	session.artifactDetection(z_threshold = 20) # CHECK METHOD TO SPECIFY Z VALUE
...
if __name__ == '__main__':
	...
	for subject_id in [1]:	
		...
		EEG_ERP_array = [
				...
				{'timing_mem':[0.8,1.9,(0.8,1.0)]},	# tmin, tmax, baseline			
		...
		for session in EEG_run_array:						
			EEG_session = RawBDF(session['raw_data_path'])
